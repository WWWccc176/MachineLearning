...
class Agent:
    # 初始化，不复杂，参数只有仨
    def __init__(self, OOXX_Id, Epsilon, Alpha):
        self.id = OOXX_Id  # OOXX_Id 用 1 或者 2 代表是两个 Agent 当中的哪一个
        ...
        self.stored_Outcome = np.zeros(9).astype(
            np.int8
        )  # Agent 内部记录的后果，初始化为 0，表示空棋盘
    ...
    def move(self, State):
        ...
        if np.random.binomial(1, self.epsilon):  # 判断要不要采取 ε-Greedy 的随机行动
            ...
            for i in range(len(available)):  # 对每一个可能落子的地方
                ...
                temp_Outcome[available[i]] = self.id  # 假设在一个地方落子，得到后果
                temp_Value[i] = self.value[
                    tuple(temp_Outcome)
                ]  # 调用价值函数，计算得到的后果的价值
            choose = np.argmax(temp_Value)  # 选择价值最大的那一个行动
            Outcome[available[choose]] = (
                self.id
        ...
        self.value[tuple(self.stored_Outcome)] += (
            self.alpha * Error
        )  # 更新储存的（上一个）后果的价值估计
        self.stored_Outcome = (
            Outcome.copy()
        )  # 把当前的后果储存（因为接着就要进行下一步了）
        return Outcome  # 返回当前的后果


# 写一个函数判断输赢
# 这里我们直接用暴力枚举判断输赢就行了，因为状态空间就这么简单
def Judge(Outcome, OOXX_Id):  # 输入为状态和对应的玩家
    Triple = np.repeat(OOXX_Id, 3)
    winner = 0  # 默认胜负未分
    if 0 not in Outcome:  # 没地方下了
        winner = 3  # 平局
    if (
        (Outcome[0:3] == Triple).all()
        or (Outcome[3:6] == Triple).all()
        or (Outcome[6:9] == Triple).all()
    ):  # 分别判断三行
        winner = OOXX_Id
    if (
        (Outcome[0:7:3] == Triple).all()
        or (Outcome[1:8:3] == Triple).all()
        or (Outcome[2:9:3] == Triple).all()
    ):  # 分别判断三列
        winner = OOXX_Id
    if (Outcome[0:9:4] == Triple).all() or (
        Outcome[2:7:2] == Triple
    ).all():  # 分别判断两条对角线
        winner = OOXX_Id
...
# 创建两个 Agent
# 大家可以自己改变参数
Agent1 = Agent(OOXX_Id=1, Epsilon=0.1, Alpha=0.1)
Agent2 = Agent(OOXX_Id=2, Epsilon=0.1, Alpha=0.1)
    ...
    while winner == 0:  # 如果胜负未分
        ...
        if winner == 1:  # 如果 Agent1 获胜
            ...
            Agent2.value[
                ...
                -1
            )  # Agent2 对应的后果，也就是 Agent1 面临的 State 的价值对 Agent2 来说为 -1
            ...
            if winner == 2:  # 如果 Agent2 获胜
                Agent2.value[tuple(State)] = (
                    1  # Agent2 对应的后果，也就是 Agent1 面临的 State 的价值对 Agent2 来说为 1
                )
                Agent1.value[tuple(Outcome)] = -1  # Outcome 的价值对 Agent1 来说为 -1
